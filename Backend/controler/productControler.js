...
exports.addProduct = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "FILL ALL THE REQUIRED FIELDS AND ENSURE IMAGE IS IN CORRECT FORMAT" });
  }
};
...
exports.getProductsByIds = async (req, res) => {
  try {
    ...
    if (invalidIds.length > 0) {
      return res
        .status(400)
        .json({ message: "Invalid product IDs", invalidIds });
    }
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    try{
      await user.userProducts.pull(id);
      await user.save();
    }catch{
