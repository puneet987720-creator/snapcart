...
export const CartPage = () => {
  ...
  // Ensure cartItems is always an array to avoid mobile/runtime errors
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {items.map((item, index) => (
              ...
              >
                <figure>
                  <img
                    src={`${UPLOADS_URL}/${item?.productId?.image || 'placeholder.png'}`}
                ...
                <div className="card-body">
                  <h2 className="card-title">{item?.productId?.name || 'Unnamed product'}</h2>
                  <p className="text-sm text-gray-500">Brand: {item?.brand || '—'}</p>
                  <p className="text-sm">Price: ₹{item?.productId?.price ?? 0}</p>
